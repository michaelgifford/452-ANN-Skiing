...
parser = argparse.ArgumentParser(
    description="Train a DQN net to play Atari Skiing.")
...
done = True  # env must be reset

# Deep Q Networks (target & online)
input_height = 88
input_width = 80
input_channels = 1
...
n_outputs = env.action_space.n  # numer of discrete actions available
...
# OP copying online DQN to target DQN
...
def epsilon_greedy(q_values, step):
    ...
    if np.random.rand() < epsilon:
        return np.random.randint(n_outputs) # rand. action
    else:
...
def preprocess_observation(obs):
    img = obs[1:176:2, ::2] # Crop & down size
    img = img.mean(axis=2) # Convert greyscale
    img[img==mspacman_color] = 0 # Greyscale contrast improvement
    img = (img - 128) / 128 - 1 # Normalize (-1. to 1.)
    return img.reshape(88, 80, 1)

# Tensorflow execution
training_start = 10000  # iterations after which training starts
discount_rate = 0.99
skip_start = 90  # time skipped at start
batch_size = 50
iteration = 0  # game iterations
done = True # env must be reset

# Max Q-Value/time & calculate mean/game
loss_val = np.infty
game_length = 0
total_max_q = 0
mean_max_q = 0.0
...
with tf.Session() as sess:
    ...
    while True:
        ...
        if done: # Game done, restart
            obs = env.reset()
            for skip in range(skip_start): # Skip start each game iteration
        ...
        # Online DQN evaluate what to do
        ...
        # Calculate stats for keeping track of progress
        ...
        if iteration < training_start or iteration % args.learn_iterations != 0:
            continue # only train after the warmup period and at reg. intervals
        
        # Sample memories & use target DQN to produce target Q-Value
        ...
        # Regularly copy online DQN to target DQN
